package com.example.test.drools.converter;

import com.example.test.drools.service.TravelRuleInterface;
import com.example.test.drools.model.RuleResult;
import com.example.test.utils.KieSessionUtil;
...
public class TravelRuleGenerate {
    ...
    private List<String> generateWhenContent() {
        ...
        List<String> ruleList = Splitter.on(Pattern.compile("AND")).trimResults().splitToList(rule);
        
        List<String> rules = ruleList.stream().map(this::generateARule).collect(Collectors.toList());
        return rules;
    }
